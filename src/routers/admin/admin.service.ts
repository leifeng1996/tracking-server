...
export class AdminService {
  ...
  async findGuestUserPage(offset: number, limit: number, where?: any): Promise<any> {
    let match: any = {};
    let pipelineMatch: any = {};
    if (!!where && !!where.account) {
      const user = await this.tableGuestModel.findOne({
    ...
    return await this.tableGuestModel.aggregate([...[{ $match: match}], ...pipeline,
      ...
      const statistical = await this.tableGuestModel.aggregate([...[{ $match: match}], ...pipeline,
        ...
        values.companyEarnings = ((values.totalWin >= 0 ? 0 - values.totalWin : Math.abs(values.totalWin)) * game_gold_multiple) - (values.washCodeCost * game_gold_multiple) - (values.shareEarnings * game_gold_multiple);
        values.companyEarnings /= game_gold_multiple;
        return values;
      });
      return {
        statistical,
  ...
  async findTableGuestBorrowing(where?: any): Promise<any> {
    let match: any = {};
    if (!!where && !!where.account) {
      const user = await this.tableGuestModel.findOne({
        account: where.account
  ...
  async findTableGuestBorrowingPage(offset: number, limit: number, where?: any): Promise<any> {
    let match: any = {};
    if (!!where && !!where.account) {
      const user = await this.tableGuestModel.findOne({
        account: where.account
  ...
  async findGuestBetting(where?: any): Promise<any> {
    ...
    return this.tableGuestBetting.aggregate([
      ...
      const statistical = await this.tableGuestBetting.aggregate([
        ...
        let obj = {
          totalWin: 0, totalBet: 0, totalValidBet: 0,
          totalWinCash: 0, totalWinChip: 0,
          totalWashCode: 0, totalWashCodeCost: 0
        };
        for (let i = 0; i < rs.length; ++i) {
          let item = rs[i];
          let user = item.user[0];
          obj.totalBet += item.userBetMoney;
          obj.totalValidBet += item.validBetMoney;
          if (item.type === 0)
            obj.totalWinCash += item.settlementMoney;
          else
            obj.totalWinChip += item.settlementMoney;
          obj.totalWashCodeCost += Math.abs(item.settlementMoney) * user.ratio;
        }
        obj.totalWashCodeCost /= game_ratio_multiple;
        obj.totalWashCodeCost = parseFloat(obj.totalWashCodeCost.toFixed(2))
  ...
  async findGuestBettingPage(offset: number, limit: number, where?: any): Promise<any> {
    ...
    return this.tableGuestBetting.aggregate([
      ...
      const statistical = await this.tableGuestBetting.aggregate([
        ...
        let obj = {
          totalWin: 0, totalBet: 0, totalValidBet: 0,
          totalWinCash: 0, totalWinChip: 0, totalWashCodeCost: 0
        };
        for (let i = 0; i < rs.length; ++i) {
          let item = rs[i];
          let user = item.user[0];
